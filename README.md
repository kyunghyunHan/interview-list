...
DID란 개인 정보를 사용자의 단말기에 저장해, 개인 정보 인증 시 필요한 정보만 골라서 제출하도록 해주는 전자신원증명 기술입니다. 
```
11.  코인 및 토큰 이코노미
```
  '토큰과 그것이 쓰여질 실물 경제시스템 사이에 규칙을 설계’하는 것, 토큰이라는 보상으로 다양한 참여자들이 가치를 주고받고 기여하는 경제입니다
```
12. 블록체인 프라이버시 문제 어떻게 해결? 
```
